...
# Стоимость создания отклика в токенах (fallback если Runner не отправил track_cost)
APPLY_FALLBACK_COST = 1
    ...
    async def apply_to_vacancy(
        ...
        if success:
            ...
            if self._track_cost is None:
                logger.warning(
                    f"Track completed without cost data, using fallback cost: {APPLY_FALLBACK_COST}"
                )
                self._track_cost = APPLY_FALLBACK_COST
                # Для fallback не применяем multiplier, используем прямое значение
                final_cost = float(APPLY_FALLBACK_COST)
                multiplier_used = 1.0
            else:
